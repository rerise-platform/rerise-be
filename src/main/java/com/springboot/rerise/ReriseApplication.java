...
public class ReriseApplication {
    ...
    public static void main(String[] args) {
        ...
        // 2. 환경변수 등록 (null 체크 포함)
        setPropertySafely("DB_URL", dotenv.get("DB_URL"));
        setPropertySafely("DB_USER", dotenv.get("DB_USER"));
        setPropertySafely("DB_PASS", dotenv.get("DB_PASS"));
        setPropertySafely("JWT_SECRET", dotenv.get("JWT_SECRET"));
        setPropertySafely("GEMINI_API_KEY", dotenv.get("GEMINI_API_KEY"));
        setPropertySafely("PERPLEXITY_API_KEY", dotenv.get("PERPLEXITY_API_KEY"));
    ...
    /**
     * 환경변수를 안전하게 설정하는 헬퍼 메서드
     */
    private static void setPropertySafely(String key, String value) {
        if (value != null) {
            System.setProperty(key, value);
        } else {
            System.err.println("Warning: Environment variable " + key + " is not set in .env file");
        }
    }
